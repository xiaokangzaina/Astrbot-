...
安装包名称：astrbot_plugin_permission_controller-v1.8.0.zip

Release 地址：

https://github.com/xiaokangzaina/astrbot_plugin_permission_controller/releases/tag/v1.8.0
...
v1.8.0 起，插件配置页排版分为两个大类：私聊类和群聊类。旧版平铺配置仍然兼容，不需要手动迁移。
...
| admin_bypass | bool | 管理员绕过限制。开启后，AstrBot 全局管理员不会被本插件拦截。该开关同时影响私聊和群聊。 |
...
| allowed_groups | list | 群聊整体放行列表。填写群号，并会同步到 AstrBot 平台白名单。 |
| simple_rules | list | 用户-群号组合放行规则，格式为用户 QQ 与群号组合。 |
...
- 插件配置页排版改为“私聊类”和“群聊类”两个大类。
- 兼容旧版平铺配置和新版分组配置。
- 移除日志过滤器，恢复后台显示机器人回复消息日志。
